...
Fe65p2Xtalk3x3MaskLoop::Fe65p2Xtalk3x3MaskLoop() : LoopActionBase() {
    min = 0;
...
void Fe65p2Xtalk3x3MaskLoop::init() {
...
void Fe65p2Xtalk3x3MaskLoop::end() {
    // Nothing to do
}

void Fe65p2Xtalk3x3MaskLoop::execPart1() {
...
void Fe65p2Xtalk3x3MaskLoop::execPart2() {
   m_cur += step;
   if (!(m_cur<max)) m_done = true;
}

// Get injection mask words
std::pair<uint16_t, uint16_t> Fe65p2Xtalk3x3MaskLoop::getMaskInj(unsigned col, unsigned row){
...
std::pair<uint16_t, uint16_t> Fe65p2Xtalk3x3MaskLoop::getMaskEn(unsigned col, unsigned row, int shift){
  std::pair<uint16_t, uint16_t> m_out;
...
uint16_t Fe65p2Xtalk3x3MaskLoop::getRowMask(unsigned row, unsigned word){
  if (row >= 0 && row < 4) {
...
uint16_t Fe65p2Xtalk3x3MaskLoop::getColMask(unsigned col, unsigned word){
  if (col >= 0 && col < 4) {
...
int Fe65p2Xtalk3x3MaskLoop::getShift(unsigned global_col){
